package Constants;

public interface IConstants {
	/* ARENA */
	public static final int ARENA_WIDTH = 2400;//1200
	public static final int ARENA_HEIGTH = 675;
	
	/* ROBOTS */
	public static final int ROBOT_MOVEMENT_LENGTH = 1000;

	public static final int ROBOT_SPEED_DEFAULT = 9;
	
	public static final int ENERGY_PER_MOVEMENT = 1;
	public static final int WEAPONS_PER_ROBOT = 1;
	public static final int STRIKES_PER_ROBOT = 2;
	
	public static final int ENERGIA_INICIAL = 100;
	public static final int DISMINUCION_DE_ENERIA = 1;
	
	
}