...
public class Robot extends IRobot{
	...
	private MOVEMENT lastMoveUPDOWN;
	private boolean movingL,movingU = false;
	
	private int  cantidadRecorrida;
		...
		task = new TimerTask() {
			...
			public void run() { 
				...
				switch(pMove) {
				  ...
				  case LEFT:
					if (posX + movements[0] < 0) break;
					if(controller.evaluateHitboxEntidad(posX+movements[0], posY))break;
					//controller.evaluateHitboxEntidad(posX+movements[0], posY);
					posX+= movements[0];
					...
					//controller.reportMoveMulti(posX,posY);
					
				    break;
				    
				  case RIGHT:
					if (posX + movements[1] > ARENA_WIDTH-100) break;
					if(controller.evaluateHitboxEntidad(posX+movements[1], posY))break;
					//controller.evaluateHitboxEntidad(posX+movements[1], posY);
					posX+= movements[1];			
					controller.reportCoords(posX, posY);
				  ...
				  case UP:
					 if (posY + movements[0] < 0) break;
					 if(controller.evaluateHitboxEntidad(posX, posY+movements[0]))break;
					 
					 //controller.evaluateHitboxEntidad(posX, posY+movements[0]);
					 posY+= movements[0]/1.3;
					 controller.reportCoords(posX, posY);
					 controller.reportRobotImage(lastMove.getValue());
					 //this.currentOrientation = ORIENTATION.NORTH;
					 lastMoveUPDOWN = MOVEMENT.UP;
					 evaluateEnergy();
					 //System.out.println(posX + " : " +posY);
					 //controller.reportMoveMulti(posX,posY);
				  ...
				  case DOWN:
					 if (posY + movements[1] > ARENA_HEIGTH-100) break;
					 if(controller.evaluateHitboxEntidad(posX, posY+movements[1]))break;
					 
					 posY+= movements[1]/1.3;
					 controller.reportCoords(posX, posY);
					 controller.reportRobotImage(lastMove.getValue());
					 //this.currentOrientation = ORIENTATION.SOUTH;
					 lastMoveUPDOWN = MOVEMENT.DOWN;
					 evaluateEnergy();
					 //System.out.println(posX + " : " +posY);
					 //controller.reportMoveMulti(posX,posY);
	...
	public void evaluateEnergy() {
		if (this.cantidadRecorrida < ROBOT_MOVEMENT_LENGTH) {
			
			cantidadRecorrida+=ROBOT_SPEED_DEFAULT;
		}else {
			this.cantidadRecorrida = 0;
			this.energy--;
			controller.reportEnergyFrame(0, this.energy);
			controller.reportHitMulti(this.energy);
			controller.reportTrampasMulti();
		}
	}
	
	
	public boolean evaluateEnergy2() {
		if(this.energy- 1 >= 0) {
			this.energy--;
		}else {
			return false;
